...
datum
	reagents
		...
		proc/trans_to(var/obj/target, var/amount=1, var/multiplier=1)
			if(amount > total_volume) amount = total_volume
			if(amount <= 0) return
		...
		proc/trans_to_direct(var/datum/reagents/target_reagents, var/amount=1, var/multiplier=1)
					...
					if(total_matching_reagents == C.required_reagents.len)
						...
						if (!old_reactions.Find(C))
							...
							if (my_atom)
								...
								if (C.mix_sound) playsound(get_turf(my_atom), C.mix_sound, 80, 1)
		...
		proc/reaction(var/atom/A, var/method=TOUCH, var/react_volume)
				...
				if(TOUCH)
					...
					for(var/current_id in reagent_list)
						...
						if(current_reagent != null) // Don't put spawn(0) in the below three lines it breaks foam! - IM
							if(ismob(A) && !isobserver(A)) current_reagent.reaction_mob(A, TOUCH, current_reagent.volume*volume_fraction)
							if(isturf(A)) current_reagent.reaction_turf(A, current_reagent.volume*volume_fraction)
							if(isobj(A))
		...
		proc/add_reagent(var/reagent, var/amount, var/sdata, var/temp_new=T20C, var/donotreact = 0)
		...
		proc/remove_reagent(var/reagent, var/amount)
